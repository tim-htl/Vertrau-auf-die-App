...
function ProfilBearbeiten({
  ...
  const verfuegbareHobbies = useMemo(() => {
    ...
    return HOBBY_KATALOG.filter(
      (hobby) => !profil.hobbies.includes(hobby.name) && hobby.name.toLowerCase().includes(suche)
    );
  ...
  const verfuegbareModule = useMemo(() => {
    ...
    return MODUL_KATALOG.filter(
      (modul) => !profil.module.includes(modul) && modul.toLowerCase().includes(suche)
    );
  ...
  const unteresPadding =
    TAB_BAR_HOEHE_BASIS + TAB_BAR_ABSTAND_UNTEN_BASIS + insets.bottom + TAB_BAR_EXTRA_ABSTAND;
    ...
    >
      ...
      >
        ...
        {[profil.bilder.slice(0, KP_BILDER), profil.bilder.slice(KP_BILDER)].map(
          (reihe, reihenIndex) => (
            <View key={reihenIndex} style={[stile.bildReihe, reihenIndex > 0 && { marginTop: 10 }]}>
        ...
        <View style={stile.editSektion}>
              ...
              <View style={stile.frageKopfZeile}>
                ...
                <View style={stile.frageAktionen}>
                  ...
                  <TouchableOpacity
                    ...
                    style={{ opacity: index === profil.frageAntworten.length - 1 ? 0.25 : 1 }}
                  >
                    <Ionicons name="chevron-down" size={18} color="#8E8E93" />
                  </TouchableOpacity>
                  <TouchableOpacity activeOpacity={0.7} onPress={() => frageEntfernen(fa.frageId)}>
          ...
          {profil.frageAntworten.length < MAX_FRAGEN && (
            <TouchableOpacity
              activeOpacity={0.7}
              style={[stile.frageHinzufuegenKnopf, profil.frageAntworten.length > 0 && { marginTop: 12 }]}
              onPress={() => setFrageAuswahlOffen((offen) => !offen)}
            >
              <Ionicons name={frageAuswahlOffen ? "chevron-up" : "add"} size={18} color="#007AFF" />
        ...
        <View style={stile.editSektion}>
          {profil.hobbies.length > 0 && (
            <View style={stile.tagReiheEdit}>
              {profil.hobbies.map((hobby, index) => (
                ...
                >
                  <Ionicons name={hobbyIcon(hobby) as IconName} size={13} color={TEXT} style={{ marginRight: 5 }} />
          ...
          >
            <Ionicons name={hobbyAuswahlOffen ? "chevron-up" : "add"} size={18} color="#007AFF" />
            <Text style={stile.frageHinzufuegenText}>
            ...
            <>
              ...
              <View style={stile.tagReiheEdit}>
                {verfuegbareHobbies.map((hobby) => (
                  ...
                  >
                    <Ionicons name={hobby.icon as IconName} size={13} color={TEXT} style={{ marginRight: 5 }} />
                    <Text style={stile.tagEditText}>{hobby.name}</Text>
                  </TouchableOpacity>
                ))}
                {verfuegbareHobbies.length === 0 && <Text style={stile.keineTreffer}>Keine Treffer.</Text>}
              </View>
            </>
          )}
        </View>
        ...
        <View style={stile.editSektion}>
          ...
          >
            <Ionicons name={modulAuswahlOffen ? "chevron-up" : "add"} size={18} color="#007AFF" />
            <Text style={stile.frageHinzufuegenText}>
          ...
          {modulAuswahlOffen && (
            <>
              ...
              <View style={stile.tagReiheEdit}>
                ...
                {verfuegbareModule.length === 0 && <Text style={stile.keineTreffer}>Keine Treffer.</Text>}
              </View>
            </>
          )}
        </View>
        ...
        <View style={stile.editSektion}>
          {[
            ...
            <View key={label} style={[stile.readonlyZeile, index < array.length - 1 && stile.readonlyTrenner]}>
...
export default function ProfilScreen() {
  ...
  const tabBarReserviert = TAB_BAR_HOEHE_BASIS + TAB_BAR_ABSTAND_UNTEN_BASIS + insets.bottom;
  ...
  const speichernUndEditModusWechseln = useCallback(async () => {
    if (editModus) {
      ...
      try {
        ...
        Alert.alert("Speichern fehlgeschlagen", "Dein Profil konnte gerade nicht gespeichert werden.");
        return;
      }
    } else {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      ...
      headerRight: () => (
        ...
        >
          <Ionicons name={editModus ? "checkmark" : "pencil"} size={editModus ? 22 : 20} color="#007AFF" />
        </TouchableOpacity>
      ),
    });
  ...
  if (editModus) {
    return <ProfilBearbeiten profil={editProfilRef.current} onChange={onProfilAenderung} />;
  }
    ...
    <AppHintergrund>
      <ScrollView
        ...
        onMomentumScrollEnd={(event) => {
          const neueSeite = Math.round(event.nativeEvent.contentOffset.x / width);
          setSeite(neueSeite);
        }}
        scrollEventThrottle={16}
      >
        {/* WICHTIG: Die View hier umhüllt die PhotoStripCard */}
        <View style={{ width }}>
          <PhotoStripCard
            name={profil.name}
            alter={profil.alter}
        ...
        <View style={{ width, height: height - headerHeight }}>
          <ProfilAusfuehrlich
            profil={{
              name: profil.name,
              alter: profil.alter,
...
const stile = StyleSheet.create({
  ...
  headerLinksButton: { marginLeft: 12, paddingVertical: 4, paddingHorizontal: 4 },
  headerRechtsButton: { marginRight: 4, padding: 8 },

  editContainer: { flex: 1, backgroundColor: "#F2F2F7" },
  editSektionTitel: { fontSize: 13, fontWeight: "500", color: "#6D6D72", textTransform: "uppercase", letterSpacing: 0.4, marginTop: 24, marginBottom: 7, marginHorizontal: 20 },
  editSektion: { backgroundColor: "#fff", borderTopWidth: StyleSheet.hairlineWidth, borderBottomWidth: StyleSheet.hairlineWidth, borderColor: "#d1d1d6", paddingHorizontal: 16, paddingVertical: 12 },

  bildReihe: { flexDirection: "row", paddingHorizontal: 16, gap: 10 },
  bildSlot: { flex: 1, aspectRatio: 1, borderRadius: 10, overflow: "hidden" },
  bildSlotBild: { width: "100%", height: "100%" },
  bildSlotLeer: { flex: 1, backgroundColor: "#E5E5EA", justifyContent: "center", alignItems: "center" },
  bildLoeschenBadge: { position: "absolute", top: 4, right: 4, backgroundColor: "rgba(0,0,0,0.55)", borderRadius: 8, width: 16, height: 16, justifyContent: "center", alignItems: "center" },
  editHinweis: { fontSize: 12, color: MUTED, textAlign: "center", marginTop: 6, marginBottom: 2 },

  bioInput: { fontSize: 15, color: TEXT, minHeight: 80, textAlignVertical: "top", lineHeight: 21 },
  zeichenZaehler: { fontSize: 12, color: "#aaa", textAlign: "right", marginTop: 4 },

  frageKarte: { backgroundColor: "#F2F2F7", borderRadius: 12, padding: 12 },
  frageKopfZeile: { flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", gap: 8 },
  frageKopfText: { flex: 1, fontSize: 13, fontWeight: "600", color: TEXT, lineHeight: 18 },
  frageAktionen: { flexDirection: "row", alignItems: "center", gap: 10 },
  antwortInput: { fontSize: 15, color: TEXT, minHeight: 44, textAlignVertical: "top", lineHeight: 21, marginTop: 8 },
  frageHinzufuegenKnopf: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, paddingVertical: 10 },
  frageHinzufuegenText: { fontSize: 15, fontWeight: "600", color: "#007AFF" },
  frageAuswahlZeile: { paddingVertical: 12, borderTopWidth: StyleSheet.hairlineWidth, borderColor: "#d1d1d6" },
  frageAuswahlText: { fontSize: 14, color: TEXT, lineHeight: 19 },

  sucheInput: { backgroundColor: "#F2F2F7", borderRadius: 10, paddingHorizontal: 12, paddingVertical: 9, fontSize: 15, color: TEXT, marginBottom: 10 },
  tagReiheEdit: { flexDirection: "row", flexWrap: "wrap", gap: 6, marginBottom: 10 },
  tagEdit: { flexDirection: "row", alignItems: "center", backgroundColor: "#F2F2F7", borderRadius: 14, paddingHorizontal: 10, paddingVertical: 5 },
  tagAuswahl: { flexDirection: "row", alignItems: "center", backgroundColor: "#fff", borderWidth: 1, borderColor: "#d1d1d6", borderRadius: 14, paddingHorizontal: 10, paddingVertical: 5 },
  tagEditText: { fontSize: 13, color: TEXT },
  keineTreffer: { fontSize: 13, color: MUTED, paddingVertical: 4 },

  pagerDots: { position: "absolute", alignSelf: "center", flexDirection: "row", gap: 6 },
  pagerDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: "#D1D1D6" },
  pagerDotAktiv: { backgroundColor: "#007AFF" },

  readonlyZeile: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 11 },
  readonlyTrenner: { borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: "#d1d1d6" },
  readonlyLabel: { fontSize: 15, color: TEXT },
  readonlyWert: { fontSize: 15, color: MUTED, maxWidth: "60%", textAlign: "right" },
});