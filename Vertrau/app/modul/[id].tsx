...
function TeilnehmerZeile({ person, onPress }: { person: Teilnehmer; onPress?: () => void }) {
  return (
    <TouchableOpacity style={styles.zeile} activeOpacity={onPress ? 0.6 : 1} onPress={onPress}>
      {person.bild ? (
  ...
  return (
    <View style={[styles.container, { paddingTop: HEADER_HEIGHT }]}>
      ...
      <View style={styles.kopf}>
        ...
        {modul.ects !== undefined && <Text style={styles.modulMeta}>{modul.ects} ECTS</Text>}
      </View>
      <Text style={styles.sektionTitel}>Teilnehmer ({teilnehmer.length})</Text>
      <FlatList
        data={teilnehmer}
        keyExtractor={(item) => item.id}
        renderItem={({ item }) => (
          <TeilnehmerZeile person={item} onPress={item.personId ? () => router.push(`/person/${item.personId}`) : undefined} />
        )}
...
const styles = StyleSheet.create({
  ...
  modulName: { fontSize: 22, fontWeight: "700", color: "#1a1a1a" },
  modulMeta: { fontSize: 13, color: "#8E8E93", marginTop: 4 },
  ...
  liste: { backgroundColor: "#fff", borderTopWidth: StyleSheet.hairlineWidth, borderBottomWidth: StyleSheet.hairlineWidth, borderColor: "#d1d1d6" },
  zeile: { flexDirection: "row", alignItems: "center", paddingVertical: 10, paddingHorizontal: 16, gap: 12 },
  avatar: { width: 42, height: 42, borderRadius: 21 },
  avatarPlatzhalter: { backgroundColor: "#b0b0b8", justifyContent: "flex-end", alignItems: "center", overflow: "hidden" },
  name: { flex: 1, fontSize: 16, color: "#1a1a1a", fontWeight: "500" },
  trenner: { height: StyleSheet.hairlineWidth, backgroundColor: "#d1d1d6", marginLeft: 70 },
});